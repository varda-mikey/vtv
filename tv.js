...
function render(data){
  ...
  if(!data || data.active===false || (!data.imageData && !data.url)){
    setStatus('This TV screen is not assigned yet.','error'); return;
  }
  let el;
  if(data.imageData){
    ...
    if(type==='image'){
      el=document.createElement('img'); el.src=data.url; el.className='fit-'+(data.fit||'contain');
    }else if(type==='video'){
      el=document.createElement('video'); el.src=data.url; el.autoplay=true; el.loop=true; el.muted=true; el.playsInline=true; el.className='fit-'+(data.fit||'contain');
    }else{
      el=document.createElement('iframe'); el.src=data.url; el.allow='autoplay; fullscreen';
    }
  }
...
const id=screenId();
if(!id){setStatus('No screen ID. Use #1, ?id=1, or /1','error');}
else{
  ...
  onValue(ref(db,'screens/'+id), snap=>render(snap.val()), err=>setStatus('Connection error: '+err.message,'error'));
}
